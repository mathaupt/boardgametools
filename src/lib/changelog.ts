...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    description: "Fortsetzung des UI/UX-Reviews für Spiele- und Spielereihen-Bereich sowie Barcode-/Cover-Scanner: konsistente shadcn/ui-Komponenten, korrekte ARIA-Markierung, semantische Farben und verbesserte Fokuszustände.",
    changes: [
      { type: "improvement", text: "Verbleibende native `<select>`- und `<button>`-Elemente in Spiel- und Reihen-Komponenten durch shadcn/ui `Select`, `Button` und `Checkbox` ersetzt" },
      { type: "improvement", text: "Dekorative Lucide-Icons mit aria-hidden='true' versehen" },
      { type: "improvement", text: "Icon-Buttons und Eingabefelder mit passenden `aria-label`-Attributen ergänzt" },
      { type: "fix", text: "Platzhalter- und Ladetexte mit `...` durch typografische Auslassungspunkte `…` ersetzt" },
      { type: "fix", text: "`CardTitle`-Komponenten ohne Heading-Hierarchie um `as='h2'` ergänzt" },
      { type: "fix", text: "Native `<label>`/`<input>`-Checkboxen in `entry-play-details.tsx` durch shadcn/ui `Label` und `Checkbox` mit `htmlFor`/`id` ersetzt" },
      { type: "internal", text: "`cover-scan-tab.tsx` auf `next/image` mit `sizes` umgestellt und `Image`-Import-Konflikt mit Lucide gelöst" },
      { type: "internal", text: "Falsches Checkbox-Prop `onCheckedChange` in Session-Formularen auf natives `onChange` korrigiert" },
      { type: "internal", text: "Fehlende Utility-Imports für Datumsformatierung in `date-voting-section.tsx` ergänzt" },
    ],
  },
  {
    version: "0.50.1",
    date: "2026-08-10",
    title: "UI/UX- und Barrierefreiheitsfixes für Gruppen, Admin und öffentliche Seiten",
    description: "Die im UI/UX-Review identifizierten Muster wurden auf die verbleibenden Gruppen-, Admin-, Monitoring- und öffentlichen Seiten konsequent angewendet: Semantik, Fokuszustände, Datumsformatierung und konsistente Theme-Farben.",
    changes: [
      { type: "fix", text: "Hartkodierte Tailwind-Farben in `quality-tab.tsx` durch semantische Theme-Tokens ersetzt" },
      { type: "fix", text: "`toLocaleDateString`-Aufrufe in `pending-invites.tsx` und `public-discussion-card.tsx` durch `@/lib/date`-Funktionen ersetzt" },
      { type: "fix", text: "`autoFocus` in `public/group/[token]/page.tsx` entfernt" },
      { type: "fix", text: "Übrige `transition-all`-Klassen in `quality-tab.tsx` und `public-poll-card.tsx` auf spezifische Eigenschaften reduziert" },
      { type: "improvement", text: "Native `<button>`-Elemente in `game-option-picker.tsx`, `poll-card.tsx`, `public-poll-card.tsx` und `quality-tab.tsx` in `Button`-Komponenten umgewandelt" },
      { type: "improvement", text: "`CardTitle`-Komponenten in öffentlichen Gruppenseiten und Dashboard mit korrekter Heading-Hierarchie versehen" },
      { type: "improvement", text: "`AlertDialog` für deaktivierende Benutzeraktionen in `deactivate-user-modal.tsx` eingeführt" },
      { type: "improvement", text: "Icon-Buttons und Eingabefelder mit `aria-label` bzw. verbundenen Labels ergänzt" },
      { type: "improvement", text: "Ellipsen-Texte (`...`) durch typografische Auslassungspunkte (`…`) ersetzt" },
    ],
  },
  {
    version: "0.50.0",
    date: "2026-08-10",
    title: "UI/UX- und Barrierefreiheitsverbesserungen",
    description: "Verbesserungen an der Web-App-Oberfläche basierend auf einem UI/UX-Review: korrekte Link/Button-Semantik, verbesserte Fokuszustände, konsistente Datumsformatierung und Entfernung von Accessibility-Blockern.",
    changes: [
      { type: "improvement", text: "`<Button asChild>` statt `<Link><Button>` für korrekte Semantik und bessere Tastaturbedienung" },
      { type: "improvement", text: "Zentrale `Intl.DateTimeFormat`-basierte Datumsformatierung in `@/lib/date` eingeführt" },
      { type: "improvement", text: "Alle icon-only Buttons mit `aria-label` versehen" },
      { type: "improvement", text: "`tabIndex={-1}` und `autoFocus` entfernt" },
      { type: "improvement", text: "Formular-Labels mit Inputs verknüpft (`htmlFor`/`id`)" },
      { type: "improvement", text: "Übergänge von `transition-all` auf spezifische Eigenschaften reduziert" },
      { type: "fix", text: "Hartkodierte Farben in Monitoring- und Admin-Komponenten durch Theme-Tokens ersetzt" },
      { type: "fix", text: "`global-error.tsx` mit `lang=\"de\"`, Theme-Tokens und `Button` barrierefrei aufbereitet" },
      { type: "internal", text: "Spiel-Detailseite auf shadcn/ui `AlertDialog`, `Card` und `next/image` umgebaut" },
    ],
  },
  {
    version: "0.48.0",
    ...
    changes: [
      ...
      { type: "feature", text: "Mobile Game-Routen: CRUD `/api/mobile/v1/games` und Sessions pro Spiel" },
      { type: "feature", text: "Mobile Session-Routen: CRUD `/api/mobile/v1/sessions`" },
      { type: "feature", text: "Mobile Event-Routen inkl. Voting und Terminabstimmung unter `/api/mobile/v1/events`" },
      { type: "feature", text: "Mobile Group-Routen mit Umfragen, Votes und Kommentaren unter `/api/mobile/v1/groups`" },
      { type: "feature", text: "BGG-Suche, EAN-Lookup und BGG-Import ueber `/api/mobile/v1/bgg/*`" },
      { type: "feature", text: "Bild-Upload und Push-Device-Registrierung fuer iOS-Geraete" },
      { type: "feature", text: "Oeffentliche Event-Teilnahme via `/api/mobile/v1/public/event/[token]`" },
      { type: "feature", text: "Sign in with Apple ueber `/api/mobile/v1/auth/apple` mit JWT-Verifikation" },
      { type: "improvement", text: "FEATURES.md, CONCEPT.md und OpenAPI-Spezifikation um Mobile API erweitert" },
  ...
  },
  {
    ...
    description: "Erstes lauffähiges Xcode-Projekt für die native iOS-Companion-App mit SwiftUI, SwiftData-Modellen, Keychain-gesicherten API-Tokens, APIClient, AuthManager und ersten Übersichts-Ansichten.",
    changes: [
      { type: "feature", text: "iOS-Projekt `BoardGameTools/` mit XcodeGen erzeugt (Swift 6, iOS 17+, SwiftUI)" },
      { type: "feature", text: "APIClient mit Bearer-Token, automatischem Refresh, Upload und deutschen Fehlermeldungen" },
      { type: "feature", text: "KeychainManager und AuthManager für sichere Token-Speicherung und Anmeldung" },
      { type: "feature", text: "SwiftData-Modelle für Spiele, Sessions, Events, Gruppen, Benutzer und Sync-Metadaten" },
      { type: "feature", text: "LoginView, DashboardView, GameListView, SessionListView, EventListView, GroupListView und SettingsView" },
      { type: "internal", text: "Xcode-Build und Unit-Test-Ziel erfolgreich im iPhone-16-Simulator getestet" },
    ],
  },
  {
    version: "0.47.4",
